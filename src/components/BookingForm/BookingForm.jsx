...
function BookingForm({ user, selectedRoom, setSelectedRoom }) {
  ...
  const addRoombookings = (roombooking) =>
    setBookings([roombooking, ...bookings]); // add new room booking
  ...
  if (roomBookingData.bookingStartDate && roomBookingData.bookingEndDate) {
    numberOfNights = differenceInCalendarDays(
      new Date(roomBookingData.bookingEndDate),
      new Date(roomBookingData.bookingStartDate)
    );
  }
      ...
      <div className="drawer drawer-mobile">
        ...
        <div className="drawer-content flex flex-col items-center justify-start">
          <div className="form-control w-full max-w-xs">
            ...
            <h1 name="roomsName" className="hidden">
            ...
            <h1 className="text-xl">
              Customer Name:
              <span className="text-xl font-bold py-2"> {user.name}</span>
            </h1>
            ...
            <div>
              <label className="label label-text text-xl">
                Check Out date{" "}
              </label>
              <input
                type="date"
            ...
            {/* if numberOfNights is 0, show error */}
            <div>
              {numberOfNights <= 0 && (
                <div>
                  <p className="text-xl">
                    *Error <span> {numberOfNights} night selected</span>
                  </p>
                  <p className="text-xl">A min of 1 night is required.</p>
                </div>
              )}
              {/* if numberOfNights is more than 0, numbers of night selected */}
              {numberOfNights > 0 && (
                <p className="my-3">
                  Numbers of nights selected:{" "}
                  <span className="font-bold"> {numberOfNights}</span> nights
                </p>
              )}
              <br />
              {/* calculate the price based on number Of Nights selected and allow user to continue the booking*/}
              {numberOfNights > 0 && (
                <div>
                  <p className="text-xl mt-2">
                    {" "}
                    Total Estimated Price: S${TotalPrice}{" "}
                  </p>
                  <br />
                  {pets.length === 0 ? (
                    <div className="text text-center">
                      <h1 className="text-xl font-bold">
                        You have not register any Pet yet.
                      </h1>
                      <h1 className="text-xl font-bold my-5">
                        Please add before booking a room reservations.
                      </h1>
                      <Link to={`/pets/new`}>
                        {" "}
                        <button className="btn btn-outline btn-secondary btn-xs sm:btn-sm md:btn-md lg:btn-lg">
                          Add My Paw Buddy's information
                        </button>
                      </Link>
                    </div>
                  ) : (
                    <div>
                      <label className="label label-text text-xl mt-5">
                        Paw Guest Name
                      </label>
                      <select
                        name="petsName"
                      ...
                      >
                        <option disabled selected className="text-xl">
                          Select the pet who is staying
                        </option>
                        {pets.map((p) => (
                          <option className="text-xl" value={p._id}>
                            {p.petName}
                          </option>
                        ))}
                      </select>
          ...
          <Link
            to={`/bookings/TermsAndConditions`}
            className="link link-info py-2 text-xl font-bold"
          >
          ...
          <button
            type="submit"
            className="btn btn-secondary cursor-pointer btn-xs sm:btn-sm md:btn-md lg:btn-lg"
            disabled={disable}
            onClick={handleNewRoomBooking}
          >
            Confirm Booking
          </button>
          <br />
          <Link to={`/rooms`}>
            <button className="btn btn-outline btn-warning cursor-pointer btn-xs sm:btn-sm md:btn-md lg:btn-lg">
              Returns to Rooms
            </button>
          </Link>
        </div>
